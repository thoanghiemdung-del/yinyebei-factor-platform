"""Minimal backfill — every write verified, no abstraction."""
import sys, os, time, json, uuid, datetime, math, gc, traceback
import numpy as np
...
# Get starting DB count from a fresh connection
c0 = sq.connect(DB)
db_start = c0.execute('SELECT COUNT(*) FROM alpha_history').fetchone()[0]
c0.close()
print(f'Starting DB count: {db_start}', flush=True)
...
for idx, expr in todo:
    ...
    if (done + errors) % 10 == 0:
        ...
        print(f'[{done+errors}/{n_todo}] {pct:.0f}% done={done} err={errors} '
              f'db={now} (+{delta}) elapsed={elapsed/60:.0f}min', flush=True)
        last_db = now
        gc.collect()

elapsed = time.time() - t0
final = sq.connect(DB).execute('SELECT COUNT(*) FROM alpha_history').fetchone()[0]
print(f'DONE: {done} saved, {errors} errors in {elapsed/60:.0f}min. DB: {final} (started {db_start})', flush=True)
