"""Agent M4: Advanced combos + group_neutralize + smart_money (25 factors)"""
...
('rank(signed_power(ret_20d,0.5))','根号20d动量','market_cap'),
('-rank(signed_power(ret_5d,2))','平方5d反转','market_cap'),
...
for i,(e,n,neut) in enumerate(f):
    ...
    try:
        ...
        print(f'M4.{i+1} {n} IC={ic:.4f} S={sh:.2f}')
    except Exception as ex:print(f'M4.{i+1} {n} FAIL:{str(ex)[:30]}')
print('Agent M4 done')
