# -*- coding: utf-8 -*-
"""Build complete dashboard with all features. No CDN, no external deps."""
import os
...
.tb{padding:6px 14px;background:#161b22;border:1px solid #30363d;border-radius:6px 6px 0 0;cursor:pointer;font-size:11px;color:#8b949e}
.tb.on{background:#111820;color:#e94560;border-color:#e94560;border-bottom-color:#111820;font-weight:bold}
.pn{background:#111820;border:1px solid #30363d;border-radius:0 6px 6px 6px;padding:12px;margin-bottom:12px;display:none}
.pn.on{display:block}
...
.row .rbtn{background:#e94560;color:#fff}
.row .rbtn:disabled{background:#555}
.row .sbtn{background:#ff6b6b;color:#fff;display:none}
.prog{height:3px;background:#30363d;border-radius:2px;margin-bottom:6px;overflow:hidden;display:none}
.prg div{height:100%;background:#58a6ff;width:0;transition:width 0.3s}
...
<div class="prog" id="p-0"><div></div></div><div class="err" id="er-0"></div></div>
...
<div class="prog" id="p-1"><div></div></div><div class="err" id="er-1"></div></div>
...
<div class="prog" id="p-2"><div></div></div><div class="err" id="er-2"></div></div>
...
<h2>快速上手</h2><p>输入表达式→点<b>回测</b>→等~10秒→指标更新。3个标签页可同时跑不同因子。支持市值中性化。</p>
<h2>常用字段</h2><table><tr><th>字段</th><th>含义</th></tr><tr><td><code>close</code></td><td>收盘价</td></tr><tr><td><code>open</code></td><td>开盘价</td></tr><tr><td><code>high/low</code></td><td>最高/最低价</td></tr><tr><td><code>volume</code></td><td>成交量</td></tr><tr><td><code>amount</code></td><td>成交额</td></tr><tr><td><code>preclose</code></td><td>前收盘价</td></tr></table>
<h2>常用算子</h2><table><tr><th>语法</th><th>含义</th></tr><tr><td><code>rank(x)</code></td><td>截面排名百分位</td></tr><tr><td><code>ts_delta(x,d)</code></td><td>x[t]-x[t-d]</td></tr><tr><td><code>ts_mean(x,d)</code></td><td>d日滚动均值</td></tr><tr><td><code>ts_std(x,d)</code></td><td>d日标准差</td></tr><tr><td><code>ts_corr(x,y,d)</code></td><td>x,y相关系数</td></tr><tr><td><code>signed_power(x,e)</code></td><td>sign(x)*|x|^e</td></tr><tr><td><code>group_neutralize(x,g)</code></td><td>分组中性化</td></tr></table>
<h2>指标说明</h2><table><tr><th>指标</th><th>含义/阈值</th></tr><tr><td>年化收益</td><td>Top10%-市场均值x250</td></tr><tr><td>Pearson IC</td><td>因子与收益相关,>0.03好</td></tr><tr><td>Sharpe</td><td>超额收益/波动,>1.0好</td></tr><tr><td>Fitness</td><td>综合评分(含换手)</td></tr><tr><td>Turnover</td><td>日换手率,<20%正常</td></tr><tr><td>最大回撤</td><td>累计最大跌幅,<50%</td></tr></table>
</div></div></div>
...
function st(i){
  sl=i;
  for(var j=0;j<3;j++){$("tab-"+j).classList.toggle("on",j===i);$("pn-"+j).classList.toggle("on",j===i);}
  if(sd[i])sm(sd[i]);
}
...
async function rsa(){
  ...
  try{var r=await fetch("/api/superalpha",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({alpha_ids:ids})});var d=await r.json();
    ...
    (d.sub_alphas||d.alphas||[]).forEach(function(sa,i){var m=sa.metrics||sa||{},nm=(sa.expression||"").substring(0,20);h+='<tr><td style="font-family:monospace;color:#58a6ff;">'+nm+'</td><td>'+((m.pearson_ic||0).toFixed(4))+'</td><td>'+((m.annual_excess||0)*100).toFixed(2)+"%</td></tr>";});
...
print(f'  Tabs: {"tab-0" in c and "pn-0" in c and "pn-1" in c}')
print(f'  SA: {"saload" in c and "rsa" in c}')
print(f'  Guide: {"快速上手" in c}')
print(f'  CDN: {"cdn" not in c.lower()}')
print(f'  Functions: go={("go(i)" in c)}, st={("st(i)" in c)}, sm={("sm(d)" in c)}')
print(f'  Metrics IDs: {all(x in c for x in ["m-excess","m-ic","m-sharpe","m-fitness","m-turnover","m-dd","m-margin","m-winrate"])}')
