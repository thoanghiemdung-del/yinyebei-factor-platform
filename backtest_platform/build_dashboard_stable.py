...
<div class="sidebar">
  <h3>新手入门指南</h3>
  <section>
    <p><strong style="color:#58a6ff">1. Alpha因子是什么</strong></p>
    <p>用数学表达式给股票打分排序，买分最高的前10%，卖分最低的后10%。持续赚钱就是Alpha。</p>
  </section>
  <section>
    <p><strong style="color:#58a6ff">2. 表达式语法</strong></p>
    <p>字段: <code>close open high low volume vwap ret_20d returns</code> 等89个</p>
    <p>时序: <code>ts_delta ts_mean ts_std ts_rank ts_corr ts_decay_linear ts_regression</code></p>
    <p>截面: <code>rank zscore signed_power group_neutralize</code></p>
    <p>详: <a href="/operators" style="color:#58a6ff">操作符</a> <a href="/data_fields" style="color:#58a6ff">字段百科</a></p>
  </section>
  <section>
    <p><strong style="color:#58a6ff">3. 指标解读</strong></p>
    <p>IC > 0.03可用 > 0.05优秀</p>
    <p>Sharpe > 1.0 | Fitness > 1.0</p>
    <p>Turnover低=成本低 | 回撤 < 5%</p>
  </section>
  <section>
    <p><strong style="color:#58a6ff">4. SuperAlpha</strong></p>
    <p><strong>等权:</strong> 勾选2-5因子→zscore→等权平均→回测→PnL叠加</p>
    <p><strong>LGB:</strong> 非线性集成→自动学习因子交互→比赛最终因子</p>
  </section>
  <div class="tip">默认: CSI800, 2020-2023, Top10%, 5日标签。多行支持: returns=close/open-1; returns+1</div>
</div>
</div>
...
function initTabs(){
  var saved=null;
  try{saved=JSON.parse(sessionStorage.getItem("dash_tabs"));}catch(e){}
  for(var i=0;i<TAB_COUNT;i++){
    var t=saved&&saved[i]&&saved[i].result?{expr:saved[i].expr,neut:saved[i].neut,result:saved[i].result,pnl:saved[i].pnl||[],ic:saved[i].ic||[]}:{expr:"rank(ts_delta(close, 20))",neut:"market_cap",result:null,pnl:[],ic:[]};
    t.chartPnl=null;tabs.push(t);
  }
  if(saved&&saved.activeTab!=null)activeTab=saved.activeTab;
  renderAllTabs();switchTab(activeTab);
  for(var i=0;i<TAB_COUNT;i++){if(tabs[i].result)drawChartsForTab(i);}
}
function saveTabs(){
  var data={activeTab:activeTab,tabs:[]};
  for(var i=0;i<TAB_COUNT;i++)data.tabs.push({expr:tabs[i].expr,neut:tabs[i].neut,result:tabs[i].result,pnl:tabs[i].pnl,ic:tabs[i].ic});
  try{sessionStorage.setItem("dash_tabs",JSON.stringify(data));}catch(e){}
}
...
function drawChartsForTab(i){
  ...
  tabs[i].chartPnl=new Chart(canvas.getContext("2d"),{type:"line",
    data:{labels:d.map(function(_,j){return j+1;}),datasets:[{label:"PnL",data:d,borderColor:"#f0883e",borderWidth:1.5,pointRadius:0,spanGaps:true,fill:true,backgroundColor:"rgba(240,136,62,0.06)"}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{display:false},y:{grid:{color:"#21262d"}}}}});
}
...
function toggleSA(i,checked){if(checked){if(saSelected.length>=5){alert("最多5个");return false;}saSelected.push(i);}else{saSelected=saSelected.filter(function(x){return x!==i;});}byId("sa-count").textContent=saSelected.length;byId("sa-run-btn").disabled=saSelected.length<2;byId("sa-lgb-btn").disabled=saSelected.length<2;}
...
async function runLGB(){
  ...
  try{
    ...
    var cm=data.metrics||{},fi=data.feature_importance||{},pnlRaw=cm.pnl_series;
    var h='<div style="color:#a855f7;font-weight:bold;margin-bottom:8px;">LightGBM ('+data.n_samples+'样本)</div>';
    h+='<div class="metrics">';
    h+='<div class="card up"><div class="val">'+((cm.annual_excess||0)*100).toFixed(2)+'%</div><div class="lbl">年化收益</div></div>';
    ...
    h+='</div>';
    if(pnlRaw&&pnlRaw.length>0)h+='<div class="chart-box"><h3>PnL</h3><canvas id="lgb-pnl-canvas" style="height:200px!important"></canvas></div>';
    h+='<div style="margin-top:6px;font-size:10px;color:#8b949e;"><strong>特征重要性:</strong> ';
    var fis=[];Object.keys(fi).forEach(function(k){fis.push([k,fi[k]]);});fis.sort(function(a,b){return b[1]-a[1];});
    fis.forEach(function(p){h+=p[0].substring(0,25)+': '+(p[1]*100).toFixed(1)+'% ';});
    h+='</div>';resEl.innerHTML=h;
    if(pnlRaw&&pnlRaw.length>0&&typeof Chart!=="undefined")setTimeout(function(){
      var ctx=document.getElementById("lgb-pnl-canvas");if(ctx)new Chart(ctx.getContext("2d"),{type:"line",
        data:{labels:downsample(pnlRaw,200).map(function(_,j){return j+1;}),datasets:[{label:"PnL",data:downsample(pnlRaw,200),borderColor:"#a855f7",borderWidth:1.5,pointRadius:0,spanGaps:true}]},
        options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{display:false},y:{grid:{color:"#21262d"}}}}});
    },100);
...
async function runSAFromHistory(){
  ...
  try{
    ...
    subs.forEach(function(sa,si){var m=sa.metrics||{},nm=(sa.expression||"").substring(0,30);
      subH+='<div class="sa-sub-item" onclick="this.nextElementSibling.classList.toggle(\'open\')"><span style="color:#58a6ff;font-family:monospace;">'+nm+'</span> <span style="color:#8b949e;">IC:'+(m.pearson_ic||0).toFixed(4)+' Ex:'+((m.annual_excess||0)*100).toFixed(1)+'% S:'+(m.sharpe||0).toFixed(2)+'</span></div>';
      subH+='<div class="sa-sub-detail"><div class="metrics">';
      subH+='<div class="card up"><div class="val">'+((m.annual_excess||0)*100).toFixed(2)+'%</div><div class="lbl">年化收益</div></div>';
    ...
    if(typeof Chart!=="undefined")setTimeout(function(){
      var ctx=document.getElementById("sa-pnl-canvas");if(ctx){
        ...
        new Chart(ctx.getContext("2d"),{type:"line",data:{labels:ds[0].data.map(function(_,j){return j+1;}),datasets:ds},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"bottom",labels:{color:"#8b949e",font:{size:10},boxWidth:12}}},scales:{x:{display:false},y:{grid:{color:"#21262d"}}}}});
      }
      subs.forEach(function(sa,si){var ctx2=document.getElementById("sa-sub-pnl-"+si);if(ctx2&&sa.pnl_series)new Chart(ctx2.getContext("2d"),{type:"line",data:{labels:downsample(sa.pnl_series,200).map(function(_,j){return j+1;}),datasets:[{label:"PnL",data:downsample(sa.pnl_series,200),borderColor:"#f0883e",borderWidth:1.5,pointRadius:0,spanGaps:true}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{display:false},y:{grid:{color:"#21262d"}}}}});});
    },100);
