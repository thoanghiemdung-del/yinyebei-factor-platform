"""Generic round runner. Usage: python run_round.py round4_factors ROUND4"""
import sys, os, json, math, time, sqlite3, uuid, datetime, importlib
...
mod_name = sys.argv[1]  # e.g. round4_factors
var_name = sys.argv[2]  # e.g. ROUND4
mod = importlib.import_module(mod_name.replace('.py',''))
factors = getattr(mod, var_name)

DB_PATH = os.path.join(script_dir, "backtest.db")

def init_db():
    db = sqlite3.connect(DB_PATH)
    db.execute("CREATE TABLE IF NOT EXISTS tested_factors (expression TEXT PRIMARY KEY, result TEXT)")
    db.commit(); db.close()

def already_tested(expr):
    db = sqlite3.connect(DB_PATH); c = db.cursor()
    c.execute("SELECT COUNT(*) FROM alpha_history WHERE expression=?", (expr,))
    na = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM tested_factors WHERE expression=?", (expr,))
    nt = c.fetchone()[0]
    db.close()
    return na > 0 or nt > 0

def mark_tested(expr, result):
    db = sqlite3.connect(DB_PATH)
    db.execute("INSERT OR IGNORE INTO tested_factors(expression, result) VALUES(?,?)", (expr, result))
    db.commit(); db.close()
...
init_db()
print(f"{mod_name}: {len(factors)} factors")
...
for expr, neut, rationale in factors:
    if already_tested(expr): skipped+=1; continue

    print(f"[{len(factors)-total-skipped-failed} left] {expr[:55]}... ", end="", flush=True)
    t0=time.time()
    try:
        ...
        ic_=metrics.get('pearson_ic')or 0
        if abs(ic_)>=0.01:
            save(expr, metrics); ok="OK"; good+=1
        else:
            mark_tested(expr, f"IC={ic_:.4f}"); ok="LOW"
        print(f"IC={ic_:.4f} [{ok}] ({time.time()-t0:.1f}s)", flush=True)
        total+=1
...
print(f"Total: {ta} alpha, {tg} >=0.01, {tg02} >=0.02")
