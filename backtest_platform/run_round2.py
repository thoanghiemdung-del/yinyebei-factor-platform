...
from round2_factors import ROUND2
...
print(f"Round 2: {len(ROUND2)} factors")
...
total=0; good=0; good02=0; skipped=0; failed=0
for expr, neut, rationale in ROUND2:
    ...
    print(f"[{len(ROUND2)-total-skipped-failed} left] {expr[:55]}... ", end="", flush=True)
    t0=time.time()
    try:
        ...
        save(expr, metrics)
        ic=metrics.get('pearson_ic')or 0; ex=metrics.get('annual_excess')or 0; ok="OK" if abs(ic)>=0.01 else "LOW"
...
db.close()
print(f"\nDone. New: {total} ({good} OK, {good02} >=0.02), skipped: {skipped}, failed: {failed}")
