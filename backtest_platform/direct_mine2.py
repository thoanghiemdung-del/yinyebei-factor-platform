#!/usr/bin/env python3
"""Direct mining — Python pipeline with FactorComputer, no Flask."""
...
def evaluate(expr):
    try:
        factor = parse_expression(expr, p, fc)
    except Exception:
        return None
    ft = factor[t0:t1]
...
def load_existing():
    ...
    rows = conn.execute(
        "SELECT id,expression,pnl_json FROM alpha_history "
        "WHERE (type='alpha' OR type IS NULL) "
    ...
    tested = set()
    for r in rows:
        tested.add(r['expression'].strip())
        try:
            pnl = json.loads(r['pnl_json'] or '[]')
            if len(pnl) < 20:
                continue
        except Exception:
            continue
        d = [pnl[i] - pnl[i - 1] for i in range(1, len(pnl))]
        ex.append({'id': r['id'], 'dailies': np.array(d)})
    return ex, tested
...
def save(expr, m):
    ...
    conn = sqlite3.connect(DB)
    conn.execute(
        'INSERT INTO alpha_history (id,name,expression,timestamp,type,metrics_json,pnl_json,ic_json,max_corr,neutralization) '
        'VALUES(?,?,?,?,?,?,?,?,?,?)',
        (eid, expr[:40], expr, ts, 'alpha', json.dumps(clean),
         json.dumps(m['pnl']), json.dumps([]), 0.0, 'market_cap'))
    conn.commit()
    conn.close()
...
for w, desc in [
    (2,'2日'),(3,'3日'),(4,'4日'),(5,'5日'),(6,'6日'),(7,'7日'),(8,'8日'),(9,'9日'),
    (10,'10日'),(12,'12日'),(15,'15日'),(20,'20日'),(30,'30日'),(40,'40日'),(60,'60日')
]:
    a(f"-rank(ts_delta(close, {w}))", f"delta_close_{w}")
    a(f"-rank(ts_delta({R}, {w}))", f"delta_ret_{w}")

for w, desc in [(3,'3日'),(5,'5日'),(8,'8日'),(12,'12日'),(20,'20日'),(30,'30日'),(40,'40日'),(60,'60日')]:
    a(f"-rank(ts_mean({R}, {w}))", f"mean_ret_{w}")
    a(f"rank(ts_mean({R}, {w}))", f"mean_ret_long_{w}")

for w in [5,10,15,20,30,60]:
    a(f"-rank(ts_std({R}, {w}))", f"std_ret_{w}")

for w in [3,5,8,12,15,20]:
    a(f"-rank(ts_sum({R}, {w}))", f"sum_ret_{w}")

for w in [5,10,20,40,60]:
    a(f"-rank(ts_rank({R}, {w}))", f"rank_ret_{w}")

for w in [3,5,10,20]:
    a(f"-rank(ts_decay_linear({R}, {w}))", f"decay_ret_{w}")
...
for w in [3,5,10,20]:
    a(f"-rank(ts_mean(volume*({R}), {w}))", f"vol_ret_{w}")
...
a("-rank(ts_delta(close, 5) * ts_delta(close, 20))", "delta5x20")
a("-rank(ts_delta({R}, 5) / (ts_std({R}, 5)+0.001))".format(R=R), "delta_ir5")
a("-rank(ts_mean((close-open)/open, 5)/ts_std((close-open)/open, 5))", "mean_ir5")
...
log(f"DONE: +{passed} | {len(existing)} total")
